...
static unsigned int idpullup_pin;
...
struct pinctrl_state *pinctrl_idpullup_high;
...
static void mt_usb_idpullup_pin_init(void)
{
#ifdef CONFIG_OF
	int ret = 0;
	...
	idpullup_pin = of_get_named_gpio(usb_node, "idpullup_gpio", 0);

	ret = gpio_request(idpullup_pin, "idpullup_gpio");
	if (ret != 0)
		pr_err("gpio_request idpullup_gpio fail\n");

	/* PAD_IDDIG pupd by DA_USB signal when usb_mode is set by DA_USB signal */
	gpio_direction_output(idpullup_pin, 1);


#endif
}

//static void mt_usb_init_idpullup(void)
//{
//	int ret = 0;
//
//	pr_debug("****%s:%d before Init IDPULLUP!!!!!\n", __func__, __LINE__);
//
//	pinctrl_idpullup_high = pinctrl_lookup_state(pinctrl, "idpullup_high");
//	if (IS_ERR(pinctrl_idpullup_high)) {
//		ret = PTR_ERR(pinctrl_idpullup_high);
//		dev_err(mtk_musb->controller, "Cannot find usb pinctrl idpullup_high\n");
//	}
//
//	pinctrl_select_state(pinctrl, pinctrl_idpullup_high);
//
//	pr_debug("****%s:%d end Init IDPULLUP!!!!!\n", __func__, __LINE__);
//
//}
...
void mt_usb_otg_init(struct musb *musb)
{
	...
	/*  */
//	mt_usb_init_idpullup();
	mt_usb_idpullup_pin_init();
}
